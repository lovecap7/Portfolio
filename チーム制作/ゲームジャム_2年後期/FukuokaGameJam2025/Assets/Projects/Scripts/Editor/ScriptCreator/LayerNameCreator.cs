...
namespace myEditor.Creator
{
    public static class LayerNameCreator
    {
        ...
        // スクリプト保存パス
        const string SAVE_PATH = "Assets/Projects/Scripts/Game/Definition/LayerName.cs";
        // ファイル名(拡張子あり)
        ...
        public static void CreateScript()
        {
            ...
            foreach (var n in InternalEditorUtility.layers.
                Select(str => new { var = RemoveInvalidChars(str), val = LayerMask.NameToLayer(str) }))
            {
                builder.Append("\t").AppendFormat(@"public const int {0} = {1};", n.var, n.val).AppendLine();
            }
            foreach (var n in InternalEditorUtility.layers.
                Select(str => new { var = RemoveInvalidChars(str), val = 1 << LayerMask.NameToLayer(str) }))
            {
